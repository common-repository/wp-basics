...
function wpb_breadcrumbs() { 
	...
	$delimiter      = $wpb_opt['delimiter']; // delimiter between crumbs 
	
	
    $show_current   = $wpb_opt['show_current']; // 1 - show current post/page/category title in breadcrumbs, 0 - don't show  

    $show_on_home   = 1; // 1 - show breadcrumbs on the homepage, 0 - don't show  

    $show_home_link = $wpb_opt['show_home_link']; // 1 - show the 'Home' link, 0 - don't show  

    $show_title     = $wpb_opt['show_title']; // 1 - show the title for the links, 0 - don't show 
	  
	
	$text['home']     = $wpb_opt['home_page_text']; // text for the 'Home' link  

    $text['category'] = $wpb_opt['cat_page_text']; // text for a category page  

    $text['search']   = $wpb_opt['search_page_text']; // text for a search results page  

    $text['tag']      = $wpb_opt['tags_page_text'];// text for a tag page  

    $text['author']   = $wpb_opt['author_page_text']; // text for an author page  

    $text['404']      = $wpb_opt['error_page_text']; // text for the 404 page
